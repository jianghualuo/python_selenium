...
class TestMark(MyTest):
    ...
    def test1_mark_as_unstar(self):
        ...
        assert(re.search("\d+", text).group() == "24"), "标记星标邮件失败"
    ...
    def test2_mark_as_unstar(self):
        ...
        self.driver.switch_to.default_content()
        text = ree.star_mail_statistics()
        assert(re.search("\d+", text).group() <= "22"), "标记星标邮件失败"
    ...
    def test3_mark_as_unstar(self):
        ...
        self.driver.switch_to.default_content()
        text = ree.star_mail_statistics()
        assert(re.search("\d+", text).group() <= "18"), "标记星标邮件失败"
    ...
    def test4_mark_as_unstar(self):
        ...
        self.driver.switch_to.default_content()
        text = ree.star_mail_statistics()
        assert(re.search("\d+", text).group() == "0"), "标记星标邮件失败"
